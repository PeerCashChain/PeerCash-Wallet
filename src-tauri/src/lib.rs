...
fn patch_and_save(app_dir: &std::path::Path, pk: &[u8], password: &str) -> Result<String, String> {
...
/// Pass-through JSON-RPC call; returns the full response object so JS can access .result / .error.
#[tauri::command]
async fn rpc_call(url: String, method: String, params: Value) -> Result<Value, String> {
    let client = reqwest::Client::new();
    let body = json!({ "jsonrpc": "2.0", "id": 1, "method": method, "params": params });
    let resp = client
        .post(&url)
...
#[tauri::command]
async fn create_account(app: tauri::AppHandle, password: String) -> Result<Value, String> {
    if password.len() < 8 {
        return Err("Password must be at least 8 characters".to_string());
    }
    let signing_key = SigningKey::random(&mut OsRng);
    let pk = signing_key.to_bytes();
    let (display_addr, _) = pk_to_address(&pk)?;
    let app_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    std::fs::create_dir_all(&app_dir).map_err(|e| e.to_string())?;
    let ks_json = patch_and_save(&app_dir, &pk, &password)?;
    Ok(json!({ "address": display_addr, "keystoreJson": ks_json }))
}
...
async fn import_private_key(
    ...
    let pk = hex::decode(pk_hex).map_err(|_| "Invalid hex private key".to_string())?;
    if pk.len() != 32 {
...
async fn import_keystore(
    ...
    let app_dir = app.path().app_data_dir().map_err(|e| e.to_string())?;
    std::fs::create_dir_all(&app_dir).map_err(|e| e.to_string())?;
    let temp = app_dir.join("_import_temp");
    std::fs::write(&temp, &keystore_json).map_err(|e| e.to_string())?;
    let pk = eth_keystore::decrypt_key(&temp, &password).map_err(|_| {
...
async fn send_transaction(
    ...
    let keystore_path = app
        ...
        .map_err(|e| e.to_string())?
        .join("keystore");
    let pk = eth_keystore::decrypt_key(&keystore_path, &password)
...
async fn start_miner(
    ...
    // Resolve binary and genesis paths: dev uses the source tree; release uses bundled copies.
    let binary_path = if cfg!(debug_assertions) {
        std::path::PathBuf::from(r"C:\Users\b_str\peercash-chain\build\bin\peercash.exe")
    } else {
    ...
    let genesis_path = if cfg!(debug_assertions) {
        std::path::PathBuf::from(r"C:\Users\b_str\peercash-chain\genesis\testnet.json")
    } else {
    ...
    if !chaindata.exists() {
        // Blocking is fine here — genesis init is a one-time <2s operation.
...
async fn stop_miner(miner_state: tauri::State<'_, MinerProcess>) -> Result<(), String> {
    // Grab the PID without holding the lock across the sleep.
    let pid = {
    ...
    if let Some(pid) = pid {
        // taskkill without /F sends a Ctrl+C console event — go-ethereum handles
        // this as SIGINT and flushes its LevelDB state before exiting.
        ...
        // Poll for exit up to 10 s (20 × 500 ms) — go-ethereum needs time to
        // finish its current block batch and flush LevelDB before exiting cleanly.
        for _ in 0..20 {
    ...
    // Timeout reached — force kill whatever is left.
    let mut guard = miner_state.0.lock().map_err(|e| e.to_string())?;
...
/// Automatically cleans up the state entry if the process has exited.
#[tauri::command]
    ...
    tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            if let tauri::WindowEvent::Destroyed = event {
                // Kill the miner process when the last window closes so it
                // doesn't become an orphan after the app exits.
